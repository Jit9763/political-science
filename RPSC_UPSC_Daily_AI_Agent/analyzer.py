...
{yt_transcript if yt_transcript else "यूट्यूब ट्रांसक्रिप्ट अभी उपलब्ध नहीं है।"}
...
4. कोचिंग शिक्षक विश्लेषण एवं यूट्यूब संपूर्ण क्लास सार (youtube_teacher_analysis): ऊपर दी गई यूट्यूब लाइव कोचिंग क्लास संपूर्ण ट्रांसक्रिप्ट (UPSC Wallah - प्रशांत सर / निर्माण आईएएस) और आउटलाइन से शिक्षक द्वारा समझाए गए सभी विषयों पर अलग-अलग विस्तृत कोचिंग कार्ड्स तैयार करें (कम से कम 6 से 10 विस्तृत कार्ड्स बनाएं)। ध्यान दें: केवल आज की क्लास में शिक्षक द्वारा पढ़ाए गए वास्तविक नए विषयों को ही लें, किसी भी पुराने या पिछले दिनों के विषय को रिपीट न करें। प्रत्येक कार्ड में:
   - शिक्षक व्याख्यान सार, अवधारणायें एवं ट्रिक्स (teacher_explanation)
   - कम से कम 3 से 4 क्लास के मुख्य बिंदु (key_takeaways)
   - परीक्षार्थी के लिए परीक्षा टिप एवं मेन्स उत्तर संरचना (exam_tip)
...
"""
    ...
    def analyze_daily_corpus(self, news_corpus):
        ...
        try:
            ...
            # Dynamically clean YouTube coaching topics directly analyzed from today's sources
            clean_yt = []
            seen_yt_topics = set()
            for item in analysis_data.get('youtube_teacher_analysis', []):
                t = item.get('topic', '').strip()
                if t and t.lower() not in seen_yt_topics:
                    seen_yt_topics.add(t.lower())
                    clean_yt.append(item)
            analysis_data['youtube_teacher_analysis'] = clean_yt
